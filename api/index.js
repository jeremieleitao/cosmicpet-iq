...
// POST /api/checkout — sauvegarde quiz data dans Firestore, retourne sessionToken + priceId pour Paddle.js
app.post('/api/checkout', async (req, res) => {
  ...
  try {
    ...
    res.json({ sessionToken, priceId: PADDLE_PRICE_ID });
  } catch (err) {
...
app.post('/api/oto-checkout', async (req, res) => {
  ...
  try {
    ...
    const checkoutUrl = `https://buy.paddle.com/?_ptxn=${txnId}`;
...
app.get('/api/payment-done', async (req, res) => {
  ...
  try {
    ...
    if (cd.project === 'astral') {
      // Paiement Astral — rediriger vers astral.thecosmicpet.com/merci
      return res.redirect(
